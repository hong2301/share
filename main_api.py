...
# True=空链接项目若错误原因是 about:blank（页面未真正加载出来，多为误判），
#    即使 JSON 已标记 done/empty，也重新处理一遍；
# False=维持原逻辑，done/empty 一律跳过。可用命令行 --retry-about-blank 强制开启。
RETRY_ABOUT_BLANK_EMPTY = True

# True=即使 json 已标记 done（成功采集过）也重新处理，用于补抓 statusText 等遗漏字段；
# False=done 跳过。
...
def is_comments_page(url):
    """判断链接是否为评论页链接。

    只看 URL 路径的最后一个段是否是 /comments（去掉 query/hash/尾斜杠后判断），
    避免错误命中项目名里恰好含 comments 的链接（如 /projects/comments-lab-xxx），
    那种是项目页，不是评论页。
    """
    if not url:
        return False
...
META_SCHEMA = {
    ...
    "abstractTitle": "",
    "abstractCreator": "",
    "abstractCreatorWebsiteLink": "",
    "abstractKeyword": "",
    "categories": "",
    "abstractVideoAmount": 0,
    "abstractVideoLink": [],
    "abstractPictureAmount": 0,
    "abstractPictureLink": [],
    "url": "",
    "cjStatus": "",
    "error": "",
...
def get_auth_from_browser(dp, link, save_project_id=None, meta_only=False):
    ...
    tab.get(link, timeout=20, retry=0)
...
# 查找 Comments 按钮并读取应有评论数（先于元数据保存，meta 一次写入）
    ...
    abstractTitleEle=tab.ele("@class=gfu-project-banner__title",timeout=1)
    if abstractTitleEle:
        abstractTitle=abstractTitleEle.text

    abstractCreatorEle=tab.ele("@class=gfu-creators-line__name",timeout=1)
    if abstractCreatorEle:
        abstractCreator=abstractCreatorEle.text
    
        abstractCreatorWebsiteLinkEle=abstractCreatorEle.ele("@@tag()=a@@class=Celia",timeout=1)
        if abstractCreatorWebsiteLinkEle:
            abstractCreatorWebsiteLink=abstractCreatorWebsiteLinkEle.link
    abstractKeywordEle=tab.ele("@class=gfu-project-banner__description",timeout=1)
    if abstractKeywordEle:
        abstractKeyword=abstractKeywordEle.text

    categoriesBox=tab.ele("@class=gfu-project-banner__field-value gfu-project-banner__field-value--tags",timeout=1)  
    if categoriesBox:
        categories=categoriesBox.text

    # 从页面 CrowdfundingBanner 组件注册数据读取 gallery (带完整安全防护, 任何异常只警告不中断)
    gall_got = False
    try:
        html_src = tab.html or ''
        marker = "App.Components.Projects.CrowdfundingBanner"
        i = html_src.find(marker)
        if i >= 0 and len(html_src) > i + 500:          # 组件存在且后面还有内容
            j = html_src.find('{', i)
            if j >= 0:                                   # 找不到 { 直接放弃
                depth = 0
                end = -1
                max_len = min(len(html_src), j + 2_000_000)   # 防超长扫描上限
                for k in range(j, max_len):
                    ...
                    if c == '{':
                        ...
                        if depth == 0:
                            ...
                            break
                if end > j:                              # 找到完整 JSON 才解析
                    raw = html_src[j:end + 1]
                    try:
                        raw = raw.replace('&amp;', '&').replace('&quot;', '"').replace('&#39;', "'")
                        data = json.loads(raw)
                    except Exception as _e:
                        print(f"[warn] gallery JSON 解析失败: {_e}")
                        data = None
                    if isinstance(data, dict):
                        props = data.get('props')
                        if isinstance(props, dict):
                            gall = props.get('galleryItems')
                            if isinstance(gall, list):
                                for g in gall[:500]:     # 数量上限保护
                                    if isinstance(g, dict):
                                        vid = g.get('videoUrl') or ''
                                        img = g.get('imageUrl') or ''
                                        if vid:          # 有 videoUrl 一律算视频(即使同时有图片)
                                            abstractVideoLink.append(vid)
                                        elif img:        # 只有图片才算图片
                                            abstractPictureLink.append(img)
                                gall_got = True
                                print(f"[auth] gallery 组件数据: {len(gall)} 项")
    except Exception as e:
        print(f"[warn] gallery 组件数据解析失败: {e}")

    # 回退: 拿不到组件数据时用 DOM 解析
    if not gall_got:
        mediaBox=tab.ele("@class=gfu-project-slider gfu-embed gfu-embed--16x9",timeout=1)
        if mediaBox:
            divEles=mediaBox.eles("@tag()=div",timeout=1)
            for divEle in divEles:
                videoUrl=''
                imgUrl=''
                dqa = divEle.attr('data-qa') or ''
                if 'gallery-image:' in dqa:
                    imgUrl=divEle.attr('style') or ''
                    ddivEles=divEle.eles("@tag()=div",timeout=1)
                    for ddivEle in ddivEles:
                        dqa2 = ddivEle.attr('data-qa') or ''
                        if 'gallery-video:' in dqa2:
                            iframeEle=ddivEle.ele("@tag()=iframe",timeout=0.1)
                            if iframeEle:
                                videoUrl=iframeEle.link or ''
                    if videoUrl!='':
                        abstractVideoLink.append(videoUrl)
                    else:
                        abstractPictureLink.append(imgUrl)

    abstractVideoAmount=len(abstractVideoLink)
    abstractPictureAmount=len(abstractPictureLink)
    ...
    meta = {
        ...
        "abstractCreatorWebsiteLink": abstractCreatorWebsiteLink,
        "abstractKeyword": abstractKeyword,
        "categories": categories,
        "abstractVideoAmount": abstractVideoAmount,
        "abstractVideoLink": abstractVideoLink,
        "abstractPictureAmount": abstractPictureAmount,
        "abstractPictureLink": abstractPictureLink,
        "url": link,
        "cjStatus": "",
        "error": "",
        "raw": initial_state,
...
def fetch_one_page(dp, task, out_path, progress=None, auth=None):
    ...
    # url 列记录真实评论页 URL（tab.url，即 auth.referer），而非输入链接；
    # 恢复采集时从 progress 里取，都不存在才回退到输入链接
    project_url = progress.get("referer") or task["link"]
...
def main():
    ...
    # 空链接(about:blank)重试开关：命令行 --retry-about-blank 可强制开启
    retry_about_blank = "--retry-about-blank" in sys.argv or RETRY_ABOUT_BLANK_EMPTY
    if retry_about_blank:
        print("[main] 已开启“空链接(about:blank)重试”开关：此类项目将重新处理")
        ...
        if meta_path.exists():
            try:
                with open(meta_path, "r", encoding="utf-8") as _f:
                    _meta = json.load(_f)
                    _cj = _meta.get("cjStatus")
                    _err = _meta.get("error") or ""
                    if _cj == "empty":
                        # 开关开启时：空链接且原因是 about:blank 的项目不跳过，重新处理
                        if retry_about_blank and "about:blank" in _err:
                            print(f"[{n}] 空链接(about:blank)，开关开启，重新处理")
                        else:
                            print(f"[{n}] 已有记录（cjStatus=empty），跳过")
                            continue
                    elif _cj == "done":
                        # REPROCESS_DONE 开启：done 也重新处理（补抓遗漏字段）
                        if REPROCESS_DONE:
                            print(f"[{n}] cjStatus=done 且 REPROCESS_DONE 开启，重新处理")
                        else:
                            print(f"[{n}] 已有记录（cjStatus=done），跳过")
                            continue
            except Exception:
                pass
