...
# 读取配置（用 python3 解析 JSON，兼容带空格的格式）
...
SHARE_DIR=$(read_config share_dir)
REMOTE=$(read_config remote); REMOTE=${REMOTE:-origin}
BRANCH=$(read_config branch); BRANCH=${BRANCH:-master}
USERNAME=$(read_config username); USERNAME=${USERNAME:-unknown}

# 替换 ~ 为 HOME
SHARE_DIR="${SHARE_DIR/#\~/$HOME}"

if [ -z "$SHARE_DIR" ]; then
    echo "Error: share_dir is empty in config.json"
    exit 1
fi
...
case "$ACTION" in
    upload)
        ...
        git pull -X ours "$REMOTE" "$BRANCH" 2>/dev/null || true
