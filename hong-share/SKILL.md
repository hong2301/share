...
配置文件路径：`<技能目录>/config.json`

即脚本同级目录的上一级，例如：
- Mac/Linux: `~/mac/巧克力加工厂/skills/hong-share/config.json`
- 通用表示: `$SKILL_DIR/config.json`

**注意**：配置文件不在 `share_dir`（如 `~/Desktop/share`）中，而是在技能目录下。

## 分支管理规则

**核心原则**：默认操作在 `master` 分支；其他分支可用，但必须用户明确指定（有明确的文本要求）才切换；用完后必须立即切回 `master`。
...
# 默认在 master；如果不在 master，切换到 master
if [ "$current_branch" != "master" ]; then
    echo "当前在 $current_branch 分支，切换到 master..."
    git checkout master
fi
...
仅在用户**明确指定分支**（有清晰的文本表述，如"把文件传到 dev 分支"）时才切换到其他分支：
...
| 检查项 | 命令 | 成功标志 |
|--------|------|----------|
| 配置文件 | 读取 `<技能目录>/config.json` | 存在且 `share_dir` 非空 |
...
| **当前分支** | `cd <share_dir> && git branch --show-current` | **默认是 master**（除非用户明确指定其他分支） |

**分支检查失败时**：
- 如果不在 master 分支且用户未指定其他分支 → 执行 `git checkout master` 后继续
- 如果有未提交的更改 → 提示用户处理后再切换
...
1. 自检通过后，默认在 `master` 分支（不在则切换；用户明确指定其他分支时用指定分支）
2. 执行 `git pull origin master` 拉取最新（在其他分支时拉取对应分支）
3. 解析用户给出的文件/目录路径
4. 检查源文件是否存在，不存在则告知用户并停止
5. 复制文件到 `share_dir`（保留原文件名，已存在则覆盖）
6. 执行对应平台的上传脚本
7. 冲突策略：使用 `git pull -X ours`，冲突时以本地上传的文件为准
8. 返回上传结果

### 2. 清空 share

1. 用户明确说"清空"时直接执行，无需二次确认
2. 默认在 `master` 分支（不在则切换；用户明确指定其他分支时用指定分支）
3. 执行对应平台的清空脚本
4. 清空逻辑：删除所有文件 → 创建空提交 → 强制推送（保留历史记录）
5. 返回清空结果

### 3. 获取文件

1. 默认在 `master` 分支（不在则切换；用户明确指定其他分支时用指定分支）
2. 执行 `git pull origin master` 拉取最新（在其他分支时拉取对应分支）
3. 执行对应平台的拉取脚本
4. 在 `share_dir` 中搜索用户指定的文件
5. 找到则返回完整路径，未找到则返回当前文件列表
