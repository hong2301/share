...
### 指令立即执行
- 用户下达的指令（上传/清空/拉取）**立即执行**，不要犹豫或自作主张
- 如果某操作有副作用需要提醒（如清空会删除所有文件、push 会强制覆盖），**在执行前提醒**一句，但不要等确认（除非用户此前明确要求过确认）

### 禁止执行用户没说的指令
- **绝不**执行用户没要求的多余操作
- 例如：用户说"清空"就只清空，**不要**清空后又自动恢复某个文件/文件夹
- **绝不**因为"上次做过"或"顺手"就自动补做其他操作
- 技能文件（SKILL.md、脚本）的本地改动**不自动同步**到 share，除非用户明确要求

### 不确定时
- 如果不确定用户意图，先询问确认，不要默认执行

> 示例：用户说"清空 share" → 只执行清空，不要清空后自己又把 hong-share 传回去。
